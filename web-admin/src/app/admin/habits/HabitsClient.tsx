...
export default function HabitsClient({ habits }: Props) {
  ...
  const handleUpload = async (habitId: string, file: File) => {
    if (!file.name.endsWith('.json')) {
      setStatus((p) => ({
        ...p,
        [habitId]: { ok: false, msg: 'Must be a .json Lottie file' },
      }));
      return;
    }
    ...
    try {
      ...
      if (json.success) {
        setStatus((p) => ({
          ...p,
          [habitId]: { ok: true, msg: 'Uploaded! Animation updated.' },
        }));
        ...
        setStatus((p) => ({
          ...p,
          [habitId]: { ok: false, msg: json.error || 'Upload failed' },
        }));
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'Upload failed';
      setStatus((p) => ({ ...p, [habitId]: { ok: false, msg: message } }));
    }
    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--text-muted)]">
          Manage Lottie animations for each behavior goal. Upload .json files
          from LottieFiles.
        </p>
      </div>
        ...
        {categories.map((cat) => (
          <button
            ...
            style={{
              background:
                filter === cat
                  ? (CATEGORY_COLORS[cat] || '#4F7CFF') + '20'
                  : undefined,
              borderColor:
                filter === cat ? CATEGORY_COLORS[cat] || '#4F7CFF' : undefined,
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        >
          ...
          <div className="text-2xl font-bold text-[#F472B6] mt-2">
            {habits.filter((h) => h.category === 'emotional').length}
          </div>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <div className="text-2xl font-bold text-[#4F7CFF] mt-2">
            {habits.filter((h) => h.category === 'social').length}
          </div>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <div className="text-2xl font-bold text-[#FB923C] mt-2">
            {habits.filter((h) => h.category === 'discipline').length}
          </div>
        </motion.div>
      </div>
        ...
        {filtered.map((h, i) => {
          ...
          return (
            ...
            >
              ...
              {status[h.id] && (
                <div
                  className={`text-xs mb-3 flex items-center gap-1 ${status[h.id].ok ? 'text-[#34D399]' : 'text-[#F87171]'}`}
                >
                  {status[h.id].ok ? (
