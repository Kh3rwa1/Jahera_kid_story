    ...
    return (
      ...
      >
        <View
          style={[
            styles.cardInner,
            { padding: cardPadding, minHeight: isTablet ? 200 : 160 },
          ]}
        >
          {isLocked && (
          ...
          <View style={styles.lottieWrap}>
            {!lottieError && lottieSource ? (
              <LottieView
                ...
                loop
                style={{
                  width: cardEmojiSize * 2.5,
                  height: cardEmojiSize * 2.5,
                }}
              />
            ) : (
              <Text style={[styles.cardEmoji, { fontSize: cardEmojiSize }]}>
                {goal.emoji}
              </Text>
            )}
          </View>
          ...
          <Text
            style={[
              styles.cardLabel,
              { fontSize: cardTitleSize, color: colors.text.primary },
            ]}
            numberOfLines={2}
          >
            {goal.label}
          ...
          <Text
            style={[
              ...
              { fontSize: cardDescSize, color: colors.text.secondary },
            ]}
          >
            {goal.description}
          </Text>
...
export const BehaviorGoalPicker = ({
  selectedGoal,
  onSelect,
  isPremium,
}: Readonly<BehaviorGoalPickerProps>) => {
  ...
  const normalizedCategories = useMemo(() => {
    const base = BEHAVIOR_CATEGORIES.map((c) =>
      typeof c === 'string'
        ? { id: c, label: c }
        : { id: String(c.id), label: String(c.label) },
    );
    return base.some((c) => c.id.toLowerCase() === 'all')
      ? base
  ...
  return (
    <View style={styles.section}>
      <Animated.View
        entering={FadeInDown.delay(50).springify().damping(18).stiffness(90)}
      >
        <ScrollView
          horizontal
          ...
          <View style={{ marginRight: cardGap }}>
            <GoalCard
              ...
              onPress={() =>
                void onPressGoal(
                  goal.id,
                  !isPremium && index % filteredGoals.length >= 4,
                )
              }
            />
          </View>
