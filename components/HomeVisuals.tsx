...
export function HeroShimmer({
  styles,
}: Readonly<{ styles: { shimmerOverlay: StyleProp<ViewStyle> } }>) {
  ...
  const shimmerStyle = useAnimatedStyle(() => ({
    transform: [
      { translateX: interpolate(x.value, [-1, 1], [-400, 400]) },
      { skewX: '-25deg' },
    ],
  }));
  return (
...
export function AnimatedStreakChip({
  count,
  styles,
}: Readonly<{
  count: number;
  styles: {
    streakChip: StyleProp<ViewStyle>;
    streakChipText: StyleProp<TextStyle>;
  };
}>) {
  ...
  return (
    <Animated.View
      style={[styles.streakChip, { backgroundColor: '#FFF3E0' }, style]}
    >
