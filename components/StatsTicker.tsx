...
const StatIcon = ({
  ...
  type: 'stories' | 'languages' | 'characters';
  color: string;
}) => {
  const size = 13;
  const strokeWidth = 2;
  switch (type) {
    ...
    case 'languages':
      return <Globe size={size} color={color} strokeWidth={strokeWidth} />;
    case 'characters':
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <View
            key={`stats-${item.label}-${idx}`}
            style={[
              styles.statsTickerPill,
              { backgroundColor: cardBackground },
            ]}
          >
            <View
              style={[
                styles.statsTickerIcon,
                { backgroundColor: item.color + '20' },
              ]}
            >
              <StatIcon type={item.type} color={item.color} />
            </View>
            <Text style={[styles.statsTickerVal, { color: textPrimary }]}>
              {item.value}
            </Text>
            <Text style={[styles.statsTickerLbl, { color: textSecondary }]}>
              {item.label}
            </Text>
          </View>
        ))}
