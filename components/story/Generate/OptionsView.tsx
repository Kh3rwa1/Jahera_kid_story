  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Section
          title="🎯 What should today's story teach?"
          colors={props.colors}
        >
          <BehaviorGoalPicker
            selectedGoal={props.selectedBehaviorGoal}
            onSelect={props.onBehaviorGoalChange}
            isPremium={isPro}
          />
        </Section>
          ...
          <View style={styles.themeGrid}>
            {THEMES.map((theme) => (
              <ThemeCard
                key={theme.id}
                theme={theme}
                selected={props.selectedTheme === theme.id}
                size={CARD_SIZE}
                colors={props.colors}
                onPress={() => props.setSelectedTheme(theme.id)}
              />
            ))}
          </View>
        </Section>
          ...
          <View style={styles.moodRow}>
            {MOODS.map((mood) => (
              <MoodCard
                key={mood.id}
                mood={mood}
                selected={props.selectedMood === mood.id}
                colors={props.colors}
                onPress={() => props.setSelectedMood(mood.id)}
              />
            ))}
          </View>
        </Section>
          ...
          <View style={styles.lengthRow}>
            {LENGTHS.map((len) => (
              <LengthCard
                key={len.id}
                len={len}
                labels={LENGTH_LABELS[len.id as 'short' | 'medium' | 'long']}
                selected={props.selectedLength === len.id}
                isPro={isPro}
                colors={props.colors}
                onPress={() =>
                  props.setSelectedLength(len.id as 'short' | 'medium' | 'long')
                }
              />
            ))}
          </View>
          ...
          >
            {SUPPORTED_LANGUAGES.map((lang) => (
              <LanguagePill
                key={lang.code}
                lang={lang}
                selected={props.selectedLanguage === lang.code}
                colors={props.colors}
                onPress={() => props.setSelectedLanguage(lang.code)}
              />
            ))}
          </ScrollView>
        </Section>
    ...
    >
      <View
        style={[
          styles.moodCard,
          {
            backgroundColor: selected ? mood.bg : colors.cardBackground,
            borderColor: selected ? mood.color : colors.text.light + '15',
          },
        ]}
      >
        ...
        <Text
          style={[
            styles.moodLabel,
            { color: selected ? mood.color : colors.text.secondary },
          ]}
        >
          {mood.label}
        </Text>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.lengthDesc, { color: colors.text.light }]}>
          {labels.desc}
        </Text>
      </View>
    </TouchableOpacity>
