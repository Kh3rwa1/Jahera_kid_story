...
import {
  deviceTTSService,
  estimateSpeechDurationMillis,
  isDeviceTtsAudioUrl,
} from '@/services/deviceTTSService';
...
const AudioProgressContext = createContext<AudioProgressType | undefined>(
  undefined,
);
...
export const AudioProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const deviceProgressTimerRef = useRef<ReturnType<typeof setInterval> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    Audio.setAudioModeAsync({
      ...
      logger.warn('[AudioContext] setAudioModeAsync error:', err),
    );
    ...
    return () => {
      ...
      if (currentSound) {
        currentSound.unloadAsync().catch((e) => {
          logger.debug('[AudioContext] unload err:', e);
        });
      }
      ...
      deviceTTSService.stop().catch((e) => {
        logger.debug('[AudioContext] device TTS stop err:', e);
      });
    };
  }, []);

  const saveProgress = useCallback(
    async (pos: number, dur: number, story: Story) => {
  ...
  const onPlaybackStatusUpdate = (status: AVPlaybackStatus) => {
    ...
    if (status.isLoaded) {
      ...
      if (
        Math.abs(newPos - lastPositionRef.current) >= 250 ||
  ...
  const loadAudioFromUrl = async (
    ...
    try {
      ...
      if (!isMountedRef.current) {
        audioSound.unloadAsync().catch((e) => {
          logger.debug('[AudioContext] unload err:', e);
        });
        return;
      }
      ...
      if (retryCount < 2 && isMountedRef.current) {
        ...
        logger.info(
          `[AudioContext] Retrying audio load in ${delay}ms (attempt ${retryCount + 2})`,
        );
    ...
    try {
      ...
      await deviceTTSService.speak(story.content, story.language_code || 'en', {
        ...
        onDone: () => {
          ...
          saveProgress(estimatedDuration, estimatedDuration, story).catch(
            (e) => {
              logger.warn(
                '[AudioContext] Failed to save device TTS completion:',
                e,
              );
            },
          );
  ...
  const loadAndPlayAudio = async (story: Story) => {
    ...
    if (audioLockRef.current) {
      logger.debug(
        '[AudioContext] loadAndPlayAudio locked, ignoring call for',
        story.id,
      );
      return;
    }
    ...
    if (isDeviceTtsAudioUrl(story.audio_url)) {
      ...
      generateAudio(story.content, story.language_code || 'en', story.id).catch(
        (err) => logger.error('[AudioContext] generateAudio error:', err),
      );
