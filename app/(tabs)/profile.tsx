...
function AnimatedAchievementCard({
  ...
  return (
    <Animated.View style={[styles.achieveCard, entrance]}>
      <Animated.View
        style={[StyleSheet.absoluteFill, { borderRadius: 28, opacity: 1 }]}
      />
      <LinearGradient
      ...
      >
        <Animated.Text style={[styles.achieveEmoji, glowStyle]}>
          {card.emoji}
        </Animated.Text>
...
function AnimatedLangProgressBar({
  pct,
  primaryColor,
  delay,
  styles,
...
const computeQuizStats = (quizAttempts: QuizAttempt[] | null) => {
  ...
  const perfectScores =
    quizAttempts?.filter((q) => q.score / q.total_questions >= 0.9).length || 0;
  const avgScore =
    totalQuizzes > 0
      ? Math.round(
          quizAttempts!.reduce(
    ...
    <Animated.View entering={FadeInDown.delay(200).springify()}>
      <View
        style={[
          styles.heroGlass,
          {
            backgroundColor: COLORS.cardBackground + '73',
      ...
      >
          ...
          <Animated.View style={[styles.heroAvatarWrap, avatarPulseStyle]}>
            ...
            {streak > 0 && (
              <Animated.View
                ...
                style={[styles.streakPin, streakPinPulseStyle]}
              >
            ...
            {topLanguage && (
              ...
              >
                <Text style={styles.langPillFlag}>
                  {getLanguageFlag(topLanguage.language_code)}
                </Text>
                <Text style={[styles.langPillName, { color: COLORS.primary }]}>
                  {topLanguage.language_name}
                </Text>
        ...
        <View
          style={[
            styles.heroStats,
            { borderTopColor: COLORS.text.light + '12' },
          ]}
        >
          {[
            {
            ...
            {
              label: 'Quizzes',
              value: String(quizStats.totalQuizzes),
              icon: <Award size={13} color={COLORS.primary} />,
            },
            {
              label: 'Streak',
              value: `${streak}d`,
              icon: <Flame size={13} color="#F59E0B" fill="#F59E0B" />,
            },
          ].map((s, i, arr) => (
...
function AchievementSection({
  ...
  stats: { avgScore: number; perfectScores: number };
  totalWords: number;
  COLORS: ColorScheme['colors'];
  styles: ProfileStyles;
}>) {
            ...
            <View key={attempt.id}>
              ...
              >
                ...
                <View style={styles.quizInfo}>
                  ...
                  />
                  <Text
                    style={[styles.quizSub, { color: COLORS.text.secondary }]}
                  >
...
function MasterySection({
  ...
  languages: { id: string; language_code: string; language_name: string }[];
  stories: Story[];
  COLORS: ColorScheme['colors'];
  styles: ProfileStyles;
}>) {
    ...
    >
      ...
      >
        {languages?.map((lang, langIdx) => {
          const langCount = (stories || []).filter(
            (s) => s.language_code === lang.language_code,
          ).length;
          const pct =
            (stories?.length || 0) > 0
              ? Math.round((langCount / stories.length) * 100)
              : 0;
          return (
            <Animated.View
              key={lang.id}
              entering={FadeInRight.delay(550 + langIdx * 100).springify()}
            >
              <View
                style={[
                  styles.langCard,
              ...
              >
                <Text style={styles.langFlag}>
                  {getLanguageFlag(lang.language_code)}
                </Text>
                <Text style={[styles.langName, { color: COLORS.text.primary }]}>
                  {lang.language_name}
                </Text>
                <Text
                  style={[styles.langCount, { color: COLORS.text.secondary }]}
                >
                  {langCount} {langCount === 1 ? 'story' : 'stories'}
                </Text>
                <AnimatedLangProgressBar
                  pct={pct}
                  primaryColor={COLORS.primary}
                  delay={700 + langIdx * 100}
                  styles={styles}
                />
              </View>
            </Animated.View>
...
function XPBanner({
  streak,
  styles,
}: Readonly<{ streak: number; styles: ProfileStyles }>) {
  const router = useRouter();
  return (
    <Animated.View
        ...
        >
          ...
          <View style={styles.xpText}>
            <View style={styles.xpBadge}>
              <Text style={styles.xpBadgeText}>LEGENDARY STREAK</Text>
            </View>
            <Text style={styles.xpTitle}>
              {streak > 0
                ? `${streak}-Day Adventure!`
...
export default function ProfileScreen() {
  ...
  const behaviorProgress = useMemo(
    () => computeBehaviorProgress(stories || [], 30),
    [stories],
  );
