...
export default function EditProfileScreen() {
  ...
  const handleAddLanguage = useCallback(
    async (langCode: string, langName: string) => {
      if (!profile) return;
      const existing = profile.languages?.find(
        (l) => l.language_code === langCode,
      );
      if (existing) return;
  ...
  const handleRemoveLanguage = useCallback(
    async (langCode: string) => {
      ...
      const success = await languageService.deleteByProfileAndCode(
        profile.id,
        langCode,
      );
      if (success) {
        updateProfile({
          languages:
            profile.languages?.filter((l) => l.language_code !== langCode) ||
            [],
        });
      }
    },
  ...
  const handleAddFamilyMember = useCallback(async () => {
    ...
    const result = await familyMemberService.add(
      profile.id,
      familyInput.trim(),
    );
    if (result) {
      updateProfile({
        family_members: [...(profile.family_members || []), result],
      });
      setFamilyInput('');
    }
  ...
  const handleRemoveFamilyMember = useCallback(
    async (id: string) => {
      ...
      if (success) {
        updateProfile({
          family_members:
            profile.family_members?.filter((m) => m.id !== id) || [],
        });
      }
    },
    [profile, updateProfile],
  ...
  const handleRemoveFriend = useCallback(
    async (id: string) => {
      ...
      if (success) {
        updateProfile({
          friends: profile.friends?.filter((f) => f.id !== id) || [],
        });
      }
    },
    [profile, updateProfile],
      ...
      >
        <Animated.View entering={FadeInDown.delay(80).springify()}>
          <View style={styles.avatarSection}>
            ...
            {saveMessage && <Text style={styles.saveMsg}>{saveMessage}</Text>}
          </View>
        </Animated.View>
            ...
            <View style={styles.nameRow}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  {
                    backgroundColor:
                      kidName.trim() !== profile.kid_name &&
                      kidName.trim().length >= 2
                        ? C.primary
                        : C.text.light + '30',
                  },
                ]}
              ...
              {profile.languages?.map((lang) => (
                <View key={lang.id} style={styles.langChip}>
                  ...
                  {(profile.languages?.length || 0) > 1 && (
                    <TouchableOpacity
                      onPress={() => handleRemoveLanguage(lang.language_code)}
                    >
            ...
            <View style={styles.chipGrid}>
              {profile.family_members?.map((m) => (
                <View key={m.id} style={styles.familyChip}>
                  ...
                  <TouchableOpacity
                    onPress={() => handleRemoveFamilyMember(m.id)}
                  >
                    <X size={14} color={C.text.light} />
                  </TouchableOpacity>
                </View>
          ...
          <View style={styles.modalContent}>
            ...
            {profile.avatar_url && (
              ...
              >
                ...
                <Text style={[styles.modalOptionText, { color: C.error }]}>
                  Remove Photo
                </Text>
              </TouchableOpacity>
            )}
